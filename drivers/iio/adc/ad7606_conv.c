				  ...
				  unsigned int *readval)
{
	...
	struct ad7606c_state *ad7606_conv = conv->phy;
...
static int ad7606_conv_set_sampling_freq(struct ad7606c_state *ad7606_conv,
					 int freq)
{
...
static int ad7606_conv_gpio_request(struct platform_device *pdev,
				    struct ad7606c_state *ad7606_conv)
{
	ad7606_conv->adc_reset = devm_gpiod_get_optional(&pdev->dev, "reset",
							 GPIOD_OUT_LOW);
	if (IS_ERR(ad7606_conv->adc_reset))
		return PTR_ERR(ad7606_conv->adc_reset);

	return 0;
}

static int ad7606_conv_full_reset(struct platform_device *pdev, struct ad7606c_state *ad7606_conv)
{
	int ret;

	ret = gpiod_direction_output(ad7606_conv->adc_reset, 1);
	if (ret) {
	...
	ret = gpiod_direction_output(ad7606_conv->adc_reset, 0);
	if (ret) {
...
static int ad7606_conv_read_raw(struct iio_dev *indio_dev,
				...
				int *val2, long info)
{
	struct ad7606c_state *ad7606_conv = iio_priv(indio_dev);
...
static int ad7606_conv_probe(struct platform_device *pdev)
{
	...
	ad7606_conv_gpio_request(pdev, st);
	...
	ad7606_conv_full_reset(pdev, st);
